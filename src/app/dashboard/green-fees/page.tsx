...
export default function GreenFeesPage() {
    ...
    return (
        <div className="flex-1 flex flex-col h-full overflow-hidden bg-background relative">
            ...
            <div className="px-4 md:px-8 py-5 flex flex-wrap items-center justify-between gap-4 shrink-0 z-10 relative mt-4 lg:mt-0">
                <div className="flex flex-wrap items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-[9px] md:text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-0.5 whitespace-nowrap">Gestión comercial de campos</p>
                    </div>
                    ...
                    <div className="flex-1 overflow-auto custom-scrollbar pr-1 md:pr-2 pb-10">
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6 items-start">
                            {loading ? (
                                ...
                                fees.map((fee) => (
                                    <div key={fee.id} className="apple-card group hover:translate-y-[-4px] transition-all duration-300 flex flex-col justify-between overflow-hidden p-6 min-h-[280px] md:h-[320px] bg-linear-to-b from-white/2 to-transparent">
                                        <div>
                                            <div className="flex justify-between items-start mb-6">
                                                ...
                                                <button
                                                    ...
                                                    className="p-2.5 md:p-3 rounded-xl bg-white/5 border border-white/5 text-gray-500 hover:text-white transition-all shrink-0"
                                                >
                    ...
                    <div className="flex-1 flex flex-col gap-6 overflow-hidden overflow-y-auto no-scrollbar pb-10">
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 shrink-0">
                            ...
                            <div className="apple-card p-6">
                                <p className="text-[10px] md:text-[11px] font-black text-gray-500 uppercase tracking-widest mb-1">Promedio Ocupación</p>
                                <h3 className="text-2xl md:text-3xl font-black text-white">74%</h3>
                                <p className="text-[10px] text-gray-500 mt-2 font-bold uppercase tracking-tight">Rendimiento óptimo general</p>
                            </div>
                            <div className="apple-card p-6 sm:col-span-2 lg:col-span-1">
                                <p className="text-[10px] md:text-[11px] font-black text-gray-500 uppercase tracking-widest mb-1">Campos Activos</p>
                                <h3 className="text-2xl md:text-3xl font-black text-white">{stats.active}</h3>
                        ...
                        <div className="flex-1 apple-card p-5 md:p-8 flex flex-col overflow-hidden min-h-[400px]">
                            <div className="flex flex-wrap items-center justify-between gap-4 mb-6 md:mb-8">
                                <div>
                                    <h3 className="text-base md:text-lg font-black text-white uppercase tracking-tight">Desempeño por Campo</h3>
                                    <p className="text-[9px] md:text-xs text-gray-500 font-bold uppercase tracking-tight">Estimación de ingresos generados</p>
                                </div>
                            </div>
                                ...
                                {courseStats.map((c, i) => (
                                    <div key={i} className="flex flex-wrap sm:flex-nowrap items-center gap-4 md:gap-6 p-4 rounded-2xl bg-white/2 border border-white/5 hover:bg-white/5 transition-all">
                                        ...
                                        <div className="text-right ml-auto">
                                            <p className="text-[8px] md:text-[9px] font-black text-gray-600 uppercase tracking-widest mb-0.5 whitespace-nowrap">Est. Mensual</p>
                                            <p className="text-xs md:text-sm font-black text-white">{formatCurrency(c.estimatedMonthlyRev)}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
            ...
            {/* NEW/EDIT MODAL */}
            {isModalOpen && (
                <div className="fixed inset-0 z-100 flex items-center justify-center bg-background/90 backdrop-blur-xl p-4 sm:p-6 overflow-hidden">
                    <motion.div
                        initial={{ opacity: 0, scale: 0.9, y: 20 }}
                        animate={{ opacity: 1, scale: 1, y: 0 }}
                        className="w-full max-w-2xl bg-[#0a1f14] rounded-2xl sm:rounded-[32px] border border-white/10 flex flex-col max-h-[95vh] sm:max-h-[90vh] overflow-hidden shadow-[0_30px_100px_rgba(0,0,0,0.5)] relative"
                    >
                        <div className="p-6 md:p-8 border-b border-white/5 flex items-center justify-between shrink-0 relative z-10">
                            ...
                            <button onClick={() => setIsModalOpen(false)} className="p-2.5 md:p-3.5 rounded-xl md:rounded-2xl bg-white/2 border border-white/5 text-gray-500 hover:text-white transition-all">
                        ...
                        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 md:p-8 space-y-5 md:space-y-6 custom-scrollbar relative z-10">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
                                <div className="space-y-2">
                                    ...
                                    <input required className="w-full bg-white/2 border border-white/5 rounded-xl md:rounded-2xl px-5 py-3 text-sm text-white focus:outline-none focus:border-primary/30 transition-all font-bold" value={editingFee?.name || ''} onChange={e => setEditingFee({ ...editingFee!, name: e.target.value })} />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[9px] md:text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Ubicación</label>
                                    <input className="w-full bg-white/2 border border-white/5 rounded-xl md:rounded-2xl px-5 py-3 text-sm text-white focus:outline-none focus:border-primary/30 transition-all font-bold" value={editingFee?.location || ''} onChange={e => setEditingFee({ ...editingFee!, location: e.target.value })} />
                                </div>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
                                <div className="space-y-2">
                                    <label className="text-[9px] md:text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Precio Semana</label>
                                    <input type="number" className="w-full bg-white/2 border border-white/5 rounded-xl md:rounded-2xl px-5 py-3 text-sm text-white focus:outline-none focus:border-primary/30 transition-all font-bold" value={editingFee?.price_weekday || 0} onChange={e => setEditingFee({ ...editingFee!, price_weekday: Number(e.target.value) })} />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[9px] md:text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Precio Fin Sem.</label>
                                    <input type="number" className="w-full bg-white/2 border border-white/5 rounded-xl md:rounded-2xl px-5 py-3 text-sm text-white focus:outline-none focus:border-primary/30 transition-all font-bold" value={editingFee?.price_weekend || 0} onChange={e => setEditingFee({ ...editingFee!, price_weekend: Number(e.target.value) })} />
                                </div>
                            </div>
                        </form>
