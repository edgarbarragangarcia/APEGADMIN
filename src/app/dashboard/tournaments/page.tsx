                                            ...
                                            {t.image_url && (
                                                <div className="h-32 md:h-40 overflow-hidden relative">
                                                    ...
                                                    <div className="absolute top-4 left-4">
                                                        <span className="px-2 py-1 rounded-md bg-[#32d74b] text-black text-[8px] font-black uppercase tracking-widest shadow-lg shadow-[#32d74b]/20">ACTIVO</span>
                                                    </div>
                                                </div>
                                            )}

                                            <div className="p-5 md:p-6 flex-1 flex flex-col">
                                                <h3 className="text-base md:text-lg font-black text-white group-hover:text-[#32d74b] transition-colors leading-tight mb-4 pr-2 uppercase tracking-tight">
                                                    {t.name}
                                                </h3>

                                                <div className="space-y-3 mb-6">
                                                    <div className="flex items-center gap-3 text-[11px] md:text-xs text-gray-400 font-bold uppercase tracking-tight">
                                                        <Calendar className="w-3.5 h-3.5 text-[#32d74b] shrink-0" />
                                                        {formatDate(t.date)}
                                                    </div>
                                                    <div className="flex items-center gap-3 text-[11px] md:text-xs text-gray-400 font-bold uppercase tracking-tight">
                                                        <MapPin className="w-3.5 h-3.5 text-[#32d74b] shrink-0" />
                                                    ...
                                                    <div className="pt-2">
                                                        <div className="flex justify-between text-[8px] md:text-[9px] font-black text-gray-500 uppercase mb-1.5 tracking-widest">
                                                            ...
                                                            <span className="text-[#32d74b]">{Math.round((t.current_participants / (t.participants_limit || 1)) * 100)}%</span>
                                                        </div>
                                                        <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
                                                            <div className="h-full bg-linear-to-r from-[#32d74b] to-primary" style={{ width: `${(t.current_participants / (t.participants_limit || 1)) * 100}%` }} />
                                                        </div>
                                                    </div>
                                                </div>
                                ...
                                {tourneyChartData.map((d, i) => (
                                    <div key={i} className="min-w-[60px] md:min-w-[80px] flex-1 flex flex-col items-center gap-4 h-full justify-end group">
                                        <div className="w-full relative flex-1 flex flex-col justify-end">
                                            <div
                                                className="w-full bg-linear-to-t from-[#32d74b]/40 to-[#32d74b] transition-all duration-300 rounded-t-lg md:rounded-t-xl group-hover:scale-x-105"
