        ...
        <div className="flex-1 flex flex-col h-full overflow-hidden bg-background relative">
            ...
            <div className="px-4 md:px-8 py-5 grid grid-cols-[1fr_auto_1fr] items-center gap-4 shrink-0 z-10 relative mt-4 lg:mt-0">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-xl md:text-2xl font-black text-foreground leading-tight uppercase tracking-tighter">Green Fees</h1>
                ...
                <div className="flex justify-end">
                    <button
                        ...
                        className="apple-button apple-button-primary apple-button-sm flex items-center justify-center shadow-lg shadow-primary/20 text-white font-bold"
                    >
                                        ...
                                        <div>
                                            <div className="flex justify-between items-start mb-6">
                                                <div className="flex flex-col min-w-0">
                                                    ...
                                                    <h3 className="text-lg md:text-xl font-black text-foreground group-hover:text-primary transition-colors leading-tight mb-2 uppercase tracking-tighter truncate">{fee.name}</h3>
                                                    <div className="flex items-center gap-2 text-[9px] md:text-[10px] text-[#5c5c5e] font-bold uppercase tracking-widest truncate">
                                                        <MapPin className="w-3 h-3 md:w-3.5 md:h-3.5 text-primary/50" /> {fee.location}
                                                    </div>
                                                </div>
                                                <button
                            ...
                            <div className="flex-1 flex flex-col gap-4 overflow-y-auto custom-scrollbar pr-1 md:pr-4">
                                {courseStats.map((c, i) => (
                                    <div key={i} className="flex flex-wrap sm:flex-nowrap items-center gap-4 md:gap-6 p-4 rounded-2xl bg-black/5 border border-black/5 hover:bg-black/10 transition-all">
                                        <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary font-black text-xs md:text-sm shrink-0">
                                            {i + 1}
                                        </div>
                                        <div className="flex-1 min-w-[150px]">
                                            <h4 className="text-[10px] md:text-xs font-black text-foreground uppercase tracking-tight mb-2 truncate">{c.name}</h4>
                                            <div className="flex items-center gap-4">
                                                <div className="flex-1 h-1.5 bg-black/5 rounded-full overflow-hidden">
                                                    <div className="h-full bg-linear-to-r from-primary to-primary/80" style={{ width: `${c.utilization}%` }} />
                                                </div>
                                        ...
                                        <div className="text-right ml-auto">
                                            ...
                                            <p className="text-xs md:text-sm font-black text-foreground">{formatCurrency(c.estimatedMonthlyRev)}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 sm:p-6 overflow-hidden">
                    ...
                    >
                        <div className="p-6 md:p-8 border-b border-black/5 flex items-center justify-between shrink-0 relative z-10">
                            ...
                            <button onClick={() => setIsModalOpen(false)} className="p-2.5 md:p-3.5 rounded-xl md:rounded-2xl bg-black/5 border border-black/5 text-[#86868b] hover:text-foreground transition-all">
                        ...
                        <div className="p-6 md:p-8 border-t border-black/5 flex items-center justify-end gap-3 md:gap-5 shrink-0 relative z-10">
                            <button onClick={() => setIsModalOpen(false)} className="px-6 md:px-8 py-3 rounded-xl md:rounded-2xl bg-black/5 text-[9px] md:text-[10px] font-black uppercase text-[#86868b] hover:text-foreground border border-black/5 transition-all">Cancelar</button>
                            <button onClick={handleSubmit} className="h-[44px] md:h-[48px] px-8 md:px-10 bg-primary text-white rounded-xl md:rounded-2xl text-[9px] md:text-[10px] font-black uppercase hover:scale-105 transition-all shadow-lg shadow-primary/20">Guardar</button>
                        </div>
                    </motion.div>
                </div>
            )}
