...
export default function UserDetailsPage() {
    ...
    if (loading) return <div className="p-8 text-foreground uppercase font-black animate-pulse">Cargando Perfil...</div>
        ...
        return (
            <div className="flex-1 flex items-center justify-center p-8">
                <div className="glass-panel p-8 text-center border-red-500/10">
                    ...
                    <h2 className="text-xl font-black text-foreground mb-2 uppercase">Usuario Extraviado</h2>
                    <button onClick={() => router.back()} className="btn-secondary px-6 text-xs font-black uppercase border-black/10">Volver</button>
                </div>
            </div>
        )
    }
    ...
    return (
        <div className="flex-1 flex flex-col h-full overflow-hidden bg-background">
            ...
            <div className="px-8 py-6 bg-background border-b border-black/5 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-4">
                    <button onClick={() => router.back()} className="p-2.5 rounded-xl bg-black/5 border border-black/5 text-gray-500 hover:text-foreground transition-all">
                        <ChevronLeft className="w-5 h-5" />
                    </button>
                    <div>
                        <h1 className="text-2xl font-black text-foreground leading-tight">{profile.full_name || 'Sin Nombre'}</h1>
                        <p className="text-[10px] text-primary font-black uppercase tracking-widest mt-0.5">Expediente de Jugador</p>
                    </div>
                </div>

                <div className="flex gap-2">
                    <button className="h-10 px-6 btn-secondary border-black/5 text-xs font-black uppercase flex items-center gap-2"><Shield className="w-4 h-4" /> Roles</button>
            ...
            <div className="flex-1 p-6 flex flex-col gap-6 overflow-hidden">
                ...
                <div className="glass-panel p-5 border-black/5 flex items-center gap-8 shrink-0 bg-linear-to-r from-primary/5 to-transparent">
                    <div className="relative">
                        <div className="w-24 h-24 rounded-2xl bg-black/5 border border-black/10 flex items-center justify-center text-3xl font-black text-foreground">
                    ...
                    <div className="flex-1 grid grid-cols-4 gap-6">
                        <div>
                            ...
                            <p className="text-xs font-black text-foreground truncate">{profile.email}</p>
                        ...
                        <div className="text-center">
                            ...
                            <p className="text-2xl font-black text-foreground">{profile.total_rounds || 0}</p>
                        </div>
                        <div className="text-right">
                            <p className="text-[9px] font-black text-gray-600 uppercase mb-1">Nivel</p>
                            <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase ${profile.is_premium ? 'bg-primary text-background' : 'bg-black/5 text-gray-500'}`}>
                ...
                <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-6 overflow-hidden min-h-0">
                    ...
                    <div className="glass-panel p-5 border-black/5 flex flex-col overflow-hidden">
                        <div className="flex items-center justify-between mb-4 shrink-0">
                            <h3 className="text-xs font-black text-foreground uppercase tracking-widest flex items-center gap-2">
                                <ShoppingBag className="w-3.5 h-3.5 text-primary" /> Recientes
                            </h3>
                            <button className="text-[9px] font-black text-gray-500 uppercase hover:text-foreground transition-colors">Ver Historial</button>
                        </div>
                        <div className="flex-1 space-y-3 overflow-hidden">
                            {recent_orders.length === 0 ? (
                                <div className="h-full flex items-center justify-center border-2 border-dashed border-black/5 rounded-2xl text-[10px] font-black text-gray-700 uppercase">Sin transacciones</div>
                            ) : (
                                recent_orders.slice(0, 3).map((order: any) => (
                                    <div key={order.id} className="p-3 bg-black/5 border border-black/5 rounded-2xl flex items-center justify-between hover:bg-black/10 transition-all group">
                                        <div className="flex items-center gap-3">
                                            <div className="p-2 rounded-xl bg-black/5 group-hover:text-primary transition-colors"><Target className="w-4 h-4" /></div>
                                            <div>
                                                <p className="text-[11px] font-black text-foreground">${Number(order.total_amount).toLocaleString()}</p>
                                                <p className="text-[9px] text-gray-600 font-bold uppercase">{new Date(order.created_at).toLocaleDateString()}</p>
                                            </div>
                                        </div>
                                        <span className="text-[8px] font-black uppercase px-2 py-0.5 bg-primary/10 text-primary border border-primary/10 rounded-full">{order.status}</span>
                                    </div>
                                ))
                            )}
                        </div>
                    ...
                    <div className="flex flex-col gap-6 overflow-hidden">
                        <div className="glass-panel p-5 border-black/5 flex flex-col flex-1">
                            <h3 className="text-xs font-black text-foreground uppercase tracking-widest mb-4 flex items-center gap-2 shrink-0">
                                <Edit className="w-3.5 h-3.5 text-primary" /> Notas CRM
                            </h3>
                            <textarea
                                className="flex-1 bg-black/5 border border-black/10 rounded-2xl p-4 text-xs text-foreground focus:outline-none focus:border-primary/30 transition-all resize-none font-bold placeholder:text-gray-500"
